...
function Index() {
  ...
  const totals = useMemo(() => {
    const pop = data.reduce((a, r) => a + r.population, 0);
    const vac = data.reduce((a, r) => a + r.vaccinated, 0);
    return { pop, vac, cov: (vac / pop) * 100 };
  }, []);
      ...
      <header className="border-b border-border bg-card">
        <div className="mx-auto max-w-6xl px-6 py-10">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">
            Tríplice viral · 1ª dose · jan–jun 2026
          </p>
          ...
          <dl className="mt-8 grid gap-4 sm:grid-cols-3">
            ...
            <Stat label="População-alvo (média mensal)" value={nf.format(totals.pop)} />
